...
%   'flux_check'    : diagnostic first-on flux check
%   %% Repeated-Rifling Option:
%   'bc'            : struct with fields t [s], h [Nt x Nz], Tg [K] to use as boundary schedule
%
% Use Example:
...
opt = struct('steel','DUPLEX','tCr_m',0,'tCr_um',[], ...
             ...
             ... %% new field to accept external BC
             'bc', []);
         
% %% track whether the user explicitly set tEnd_fd
user_set_tEnd = false;

% Case-struct declerations:
...
for k=1:2:numel(varargin)
    ...
    switch key
        ...
        case 'tend_fd',       opt.tEnd_fd = double(val); user_set_tEnd = true;  % %% PATCH
        ...
        case 'bc',            opt.bc = val;  % %% PATCH: new parser case
...
sections.names = {'S1','S3','S4','S16','S25','S30'}; % follows "chromium" article convention
...
% Chromium (optional, user defined thickness)
...
% Finite-Volume metrics (per valid cell)
...
% -------------------- ballistics / boundary conditions --------------------
%% Accept external BC schedule if provided (Repeated-Rifling); otherwise fall back to single-shot IB
fprintf('Loading boundary conditions...\n');
if ~isempty(opt.bc)
    % external schedule (repeated rifling, etc.)
    t_ib  = opt.bc.t(:);      % seconds
    h_data= opt.bc.h;         % [Nt x Nz] W/m^2K
    Tgas  = opt.bc.Tg(:);     % Kelvin
else
    % legacy single-shot path
    if exist('interiorBallistics.m','file')~=2
        error('interiorBallistics.m not found, and no ''bc'' provided.');
    end
...
% Convert Tg K→°C for wall BC math/plots (code expects °C deltas)
...
% Thermal time grid
%% if bc provided and user did not set tEnd_fd explicitly, follow the BC length
if ~isempty(opt.bc) && ~user_set_tEnd
    opt.tEnd_fd = t_ib(end);
end
t = (0:opt.dt_fd:opt.tEnd_fd).';
Nt = numel(t);

% Map each z-column to nearest section
...
%% Material Property Decleration if parameters.m does not respond
% -------------------- materials (DUPLEX + 30HN2MFA + 38HMJ + Chromium) --------------------
% Temperatures are in °C. cp in J/kg-K; rho in kg/m^3.

% Chromium (coating)
T_props   = [20 100 200 300 400 500 600 700 800 900 1000];
...
% DUPLEX
T_D      = [20 100 200 300 400 500 600 700 800 900 1000];
k_D      = [13.3 15.3 17.0 17.8 18.1 18.7 20.0 20.8 21.4 23.7 25.9];
cp_D     = [417  462  515  548  567  576  582  588  602  629  668 ];
rho_D    = [7740 7720 7690 7650 7610 7580 7540 7490 7450 7400 7340];

% 30HN2MFA 
T_k_30   = [54.2 149.1 250.0 352.0 453.3 553.6 651.1 704.4 723.0 743.3 763.0 783.1 802.8 822.9 842.8 904.9 1004.3];
k_30     = [35.9 37.3 36.0 33.8 30.9 27.2 19.7 17.1 16.0 15.8 17.1 18.7 19.3 19.5 19.7 20.3 20.6];
T_cp_30  = [38 70 100 150 200 250 300 350 400 450 500 550 600 650 700 750 800 850 900 991];
cp_30    = 1000*[0.440 0.462 0.475 0.492 0.505 0.517 0.528 0.539 0.550 0.560 0.569 0.579 0.589 0.598 0.607 0.616 0.625 0.634 0.642 0.658];
T_rho_30 = [50 100 200 400 600 700 720 725 730 735 740 750 765 770 775 785 800 900 1060];
rho_30   = 1000*[7.77 7.75 7.72 7.65 7.59 7.55 7.55 7.55 7.55 7.55 7.55 7.57 7.58 7.59 7.59 7.59 7.58 7.53 7.46];

% 38HMJ 
T_k_38   = [50.9 149.0 250.0 351.3 453.4 553.6 654.7 704.5 741.0 762.6 782.7 802.8 811.9 821.8 842.3 904.7 1004.2];
k_38     = [30.0 33.6 34.4 33.0 30.7 27.4 22.5 19.4 16.4 19.3 20.9 23.2 24.4 25.4 26.3 27.8 28.9];
T_cp_38  = [38 70 100 150 200 250 300 350 400 450 500 550 600 650 700 750 800 850 900 991];
cp_38    = 1000*[0.458 0.485 0.502 0.525 0.543 0.559 0.574 0.587 0.598 0.609 0.618 0.626 0.634 0.640 0.645 0.650 0.653 0.656 0.657 0.658];
T_rho_38 = [50 100 200 400 600 780 795 800 820 830 840 850 860 870 880 890 900 1000 1060];
rho_38   = 1000*[7.66 7.65 7.62 7.55 7.48 7.42 7.41 7.42 7.43 7.43 7.43 7.43 7.43 7.42 7.42 7.42 7.41 7.37 7.34];

% Case-Switch, Steel-Type %
steel_tag = upper(string(opt.steel));
switch steel_tag
    case "DUPLEX"
        T_k_S   = T_D;   kS_tab   = k_D;
        T_cp_S  = T_D;   cpS_tab  = cp_D;
        T_rho_S = T_D;   rhoS_tab = rho_D;
    case "30HN2MFA"
        T_k_S   = T_k_30;   kS_tab   = k_30;
        T_cp_S  = T_cp_30;  cpS_tab  = cp_30;
        T_rho_S = T_rho_30; rhoS_tab = rho_30;
    case "38HMJ"
        T_k_S   = T_k_38;   kS_tab   = k_38;
        T_cp_S  = T_cp_38;  cpS_tab  = cp_38;
        T_rho_S = T_rho_38; rhoS_tab = rho_38;
    otherwise
        warning('steel="%s" not recognized; using DUPLEX properties.', steel_tag);
        T_k_S   = T_D;   kS_tab   = k_D;
        T_cp_S  = T_D;   cpS_tab  = cp_D;
        T_rho_S = T_D;   rhoS_tab = rho_D;
end

% Interpolate for continuity
kS_fun   = @(T) interp1(T_k_S,   kS_tab,   T, 'linear','extrap');
cpS_fun  = @(T) interp1(T_cp_S,  cpS_tab,  T, 'linear','extrap');
rhoS_fun = @(T) interp1(T_rho_S, rhoS_tab, T, 'linear','extrap');
...
% first-on flux consistency flags, debug purposes
...
for n = 1:Nt-1
    ...
    % lagged properties at T^n, Picard Lag for linearization
    ...
    for j=1:opt.Nz
        ...
        for i=1:Ni
            ...
            % radial faces
            if i==1
                % inner Robin at Rin using combined resistance, see report
                % for theory background 
    ...
    % solve, w/ MATLAB sparce matrix backslash
...
% ===== helpers =====
% Harmonic Mean Thermomechanical Properties for consistency
