%% Interior-ballistics two-phase solver  (projectile travel + empty barrel)
% Arda Eren — TAIL-gating per section (projectile base)
% Variable-mass model without double-count:
% Gas inertia factor kept as phi = K (constant).
% Energy equation uses the same phi*m_eff as the motion equation.
% Clamp burn fraction: 0 ≤ zp ≤ 1 
tic
...
velocityP   = zeros(maxStep,1);   velocityP(1) = 0;
zp          = zeros(maxStep,1);
zp(1)       = 0.001;              % article IC
zetaHist    = zeros(maxStep,1);
theta       = zeros(maxStep,1);   theta(1) = R*3150;
Tgas        = zeros(maxStep,1);   Tgas(1) = 3150;
...
% run until base crosses the muzzle or time cap
while t < tEnd
    ...
    % Latch release if threshold reached this step (cheap check)
    if ~released
        % pressure at step start and end (EOS)
        P_prev = (mp*prev_state(3)*R*(prev_state(4)/R)) / ...
                 (V0 + s*prev_state(1) - mp*(1-prev_state(3))/rho_p - eta*mp*prev_state(3));
        P_new  = (mp*new_state(3)*R*(new_state(4)/R)) / ...
                 (V0 + s*new_state(1)  - mp*(1-new_state(3))/rho_p  - eta*mp*new_state(3));
        if (P_prev < p_start) && (P_new >= p_start)
            % Optional: linearized release time within the step (for reference)
    ...
    % detect if base crosses the muzzle within this step
    x_prev = prev_state(1);
    x_new  = new_state(1);
    ...
    if crossed
        ...
        pressureEqn(n) = (mp*zp(n)*R*Tgas(n)) / ...
                         (V0 + s*location(n) - mp*(1-zp(n))/rho_p - eta*mp*zp(n));
        rhoHist(n) = (mp*zp(n)) / ...
                     (V0 + s*location(n) - mp*(1 - zp(n))/rho_p - eta*mp*zp(n));
...
function [rho, pressureEqn, diffLocation, diffVelocity, diffBurnRate, diffTheta] = ...
    thermoModel(zp, Tgas, velocityP, location)
    ...
    diffTheta = ((f - R*Tgas)*mp*diffBurnRate ...
            - (gamma - 1)*phi*m_eff*velocityP.*diffVelocity) ...
            / max(mp*zp, eps);
end

% Phase-2 Function Definition (post-exit)
function [rho2, pressureEqn2, diffZp, diffZeta, diffTheta2] = ...
    thermoModelv2(zp, Tgas, zeta)
